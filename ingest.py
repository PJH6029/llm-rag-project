...
persistent_metadata_handler = util.persistent_metadata_handler
...
def download_from_s3(to_dir: str):
    import boto3
    s3_bucket = os.getenv("S3_BUCKET_NAME")
    s3_client = boto3.client("s3")
    file_names, folders = get_file_folders(s3_client, s3_bucket)
    download_files(s3_client, s3_bucket, to_dir, file_names, folders)
...
def main():
    if args.download:
        ...
        download_from_s3(to_dir=source_dir)
